...
var needCheck = {
    ...
    device: {
        name: "Central board communication test",
        id: "check_device",
        create: function() {
            return createChecking(this);
        }
    ...
    direction: {
        name: "Elevator running direction detection",
        id: "check_direction",
        create: function() {
            return createChecking(this);
        }
...
var handSocketMsg = function(e) {
    ...
    var typeMap = {
        ...
        test: function(data) {
            ...
            } else if (~data.codeInfo.indexOf("Start")) {

            } else if (~data.codeInfo.indexOf("Stop")) {
                testStep.stopStepInit();
            } else if (~data.codeInfo.indexOf("Escalator")) {
        ...
        alarm: function(data) {
            if (data.code == '-54' && ~data.codeInfo.indexOf("Test ladder")) {
                return;
            }
            if (~data.codeInfo.indexOf("direction") && ~data.codeInfo.indexOf("error")) {
                if (isTesting) {
                    testStep.stopStepInit();
                } else {
                    if (checking.p) {
...
var connect = (function() {
    ...
    function __connect() {
        ...
        connecting(function(p) {
            ...
            ws.onerror = function() {
                alert("socket The connection failed and attempts were made to reconnect
");
                p.close();
                TRY_RECONNECT++;
        ...
        ws.onclose = function(e) {
            var data = e.data;
            ws = null;
            showError1("Disconnects from the server");
            alert("Disconnects from the server");
        };
    }
